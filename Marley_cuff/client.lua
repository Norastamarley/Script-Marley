...
exports.ox_target:addGlobalPlayer({
    {
        ...
        serverEvent = "omega:Servercuff",
      
        onSelect = function(entity)
            local playerId = NetworkGetPlayerIndexFromPed(entity.entity)
            if playerId then
                ...
                }) then TriggerServerEvent('omega:Servercuff', serverId) else print('Do stuff when cancelled') end
                
            else
                print("Impossible d'obtenir l'ID serveur pour cet acteur.")
            end
        end,
...
exports.ox_target:addGlobalPlayer({
    {
        ...
        onSelect = function(entity)
            ...
            if playerId then
                ...
                --TriggerServerEvent('core_inventory:server:openInventory', serverId, 'otherplayer', nil, nil, withCloth)
            else
                --print("Impossible d'obtenir l'ID serveur.")
            end
        ...
        canInteract = function(entity, distance, coords)
            ...
            if targetId then
                TriggerServerEvent('getCuffStatus', GetPlayerServerId(targetId))
        
                RegisterNetEvent('receiveCuffedStatus')
                AddEventHandler('receiveCuffedStatus', function(status)
                    if status then
